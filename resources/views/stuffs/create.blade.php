    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('stuffs.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="harga_barang" name="harga_barang" type="number" class="mt-1 block w-full" :value="old('harga_barang')" required />
                                <x-input-error class="mt-2" :messages="$errors->get('harga_barang')" />
                            </div>

                            <div>
                                <x-input-label for="stok_barang" :value="__('Stok Barang')" />
                                <x-text-input id="stok_barang" name="stok_barang" type="number" class="mt-1 block w-full" :value="old('stok_barang')" required />
                                <x-input-error class="mt-2" :messages="$errors->get('stok_barang')" />
                            </div>

                            <div>
                                <x-input-label for="exp_barang" :value="__('Tanggal Kadaluarsa')" />
                                <x-text-input id="exp_barang" name="exp_barang" type="date" class="mt-1 block w-full" :value="old('exp_barang')" required />
                            ...
                            <div>
                                ...
                                <select id="status_ketersediaan" name="status_ketersediaan" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                        ...
                        <div>
                            ...
                            <input type="file" id="image" name="image" class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 dark:file:bg-gray-700 dark:file:text-gray-300">
                            <x-input-error class="mt-2" :messages="$errors->get('image')" />
                        </div>

                        <div>
                            <x-input-label for="keterangan_barang" :value="__('Keterangan Barang')" />
                            <textarea id="keterangan_barang" name="keterangan_barang" rows="3" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">{{ old('keterangan_barang') }}</textarea>
                        ...
                        <div class="flex items-center justify-end gap-4 border-t border-gray-200 dark:border-gray-700 pt-6">
                            <a href="{{ route('stuffs.index') }}" class="text-sm text-gray-600 dark:text-gray-400 hover:underline">
                                {{ __('Batal') }}
                            </a>
                            <x-primary-button>
                                {{ __('Simpan Barang') }}
